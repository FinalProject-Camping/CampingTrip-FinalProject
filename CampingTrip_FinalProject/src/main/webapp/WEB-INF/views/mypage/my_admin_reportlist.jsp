							...
							<tbody>
								<c:choose>
									<c:when test="${empty list }">
										<tr>
											<td colspan="5" align="center">---------- 신고내역이 없습니다 ----------</td>
										</tr>
									</c:when>
									<c:otherwise>
										<c:forEach items="${list }" var="dto">
											<tr>
												<th>${dto.reportseq }</th>
												<td><fmt:formatDate pattern="yyyy/MM/dd" value="${dto.reportdate }"/></td>
												<td>${dto.reportid }</td>
												<td>
													<a href='admin_reportdetail.do?reportseq=${dto.reportseq }'>${dto.reportcontent }</a>
												</td>
												<td>${dto.writer }</td>
											</tr>
										</c:forEach>
